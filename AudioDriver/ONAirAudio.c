...
#define										kPlugIn_BundleID				"jp.ryonihonyanagi.OnAirDeckAudio"
static pthread_mutex_t						gPlugIn_StateMutex				= PTHREAD_MUTEX_INITIALIZER;
static UInt32								gPlugIn_RefCount				= 0;
static AudioServerPlugInHostRef				gPlugIn_Host					= NULL;
static const AudioObjectPropertySelector	kPlugIn_CustomPropertyID		= 'PCst';

#define										kBox_UID						"jp.ryonihonyanagi.OnAirDeckAudio.Box"
static CFStringRef							gBox_Name						= NULL;
static Boolean								gBox_Acquired					= true;

#define										kDevice_UID						"jp.ryonihonyanagi.OnAirDeckAudio.Device"
#define										kDevice_ModelUID				"jp.ryonihonyanagi.OnAirDeckAudio.Model"
static pthread_mutex_t						gDevice_IOMutex					= PTHREAD_MUTEX_INITIALIZER;
static Float64								gDevice_SampleRate				= 48000.0;
static UInt64								gDevice_IOIsRunning				= 0;
static const UInt32							kDevice_RingBufferSize			= 65536;
static Float32								gDevice_LoopbackBuffer[65536 * 2] = { 0 };
static _Atomic UInt64						gDevice_FirstWrittenFrame		= 0;
static _Atomic UInt64						gDevice_LastWrittenFrame		= 0;
...
// Expose one stable, product-named source. The Apple sample's numbered placeholder
// names leak into conferencing apps, where the selected input source is shown in
// place of the device name.
...
static OSStatus	NullAudio_GetPlugInPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyManufacturer:
			...
			*((CFStringRef*)outData) = CFSTR("Ryo Nihonyanagi");
			*outDataSize = sizeof(CFStringRef);
			break;
		...
		case kObjectID_DataDestination_PlayThru_Master:
			switch(inAddress->mSelector)
			{
				...
				case kAudioSelectorControlPropertyItemName:
					...
					{
						*((CFStringRef*)outData) = CFRetain(CFSTR("ON AIR Deck"));
					}
					else if(inObjectID == kObjectID_DataSource_Output_Master)
					{
						*((CFStringRef*)outData) = CFRetain(CFSTR("ON AIR Deck Output"));
					}
					else
					{
						*((CFStringRef*)outData) = CFRetain(CFSTR("ON AIR Deck Monitor"));
					}
					*outDataSize = sizeof(CFStringRef);
					break;
...
static OSStatus	NullAudio_StartIO(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID)
{
	...
	else if(gDevice_IOIsRunning == 0)
	{
		...
		memset(gDevice_LoopbackBuffer, 0, sizeof(gDevice_LoopbackBuffer));
		atomic_store_explicit(&gDevice_FirstWrittenFrame, 0, memory_order_release);
		atomic_store_explicit(&gDevice_LastWrittenFrame, 0, memory_order_release);
	}
	else
	{
...
static OSStatus	NullAudio_DoIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, AudioObjectID inStreamObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo, void* ioMainBuffer, void* ioSecondaryBuffer)
{
	//	The app writes its broadcast mix to the output stream. Zoom reads the same frames from the
	//	input stream, so the device behaves as a low-latency virtual microphone.
	...
	const UInt32 frameBytes = channels * sizeof(Float32);
	UInt64 sampleFrame = (UInt64)((inOperationID == kAudioServerPlugInIOOperationReadInput)
		? inIOCycleInfo->mInputTime.mSampleTime
		: inIOCycleInfo->mOutputTime.mSampleTime);
	UInt32 startFrame = (UInt32)(sampleFrame % kDevice_RingBufferSize);
	UInt32 firstFrames = kDevice_RingBufferSize - startFrame;
	if(firstFrames > inIOBufferFrameSize) firstFrames = inIOBufferFrameSize;
	UInt32 wrappedFrames = inIOBufferFrameSize - firstFrames;

	if(inOperationID == kAudioServerPlugInIOOperationWriteMix)
	{
		memcpy(gDevice_LoopbackBuffer + startFrame * channels,
			ioMainBuffer,
			firstFrames * frameBytes);
		if(wrappedFrames > 0)
		{
			memcpy(gDevice_LoopbackBuffer,
				((Float32*)ioMainBuffer) + firstFrames * channels,
				wrappedFrames * frameBytes);
		}

		UInt64 previousEnd = atomic_load_explicit(&gDevice_LastWrittenFrame, memory_order_acquire);
		if(previousEnd == 0 || sampleFrame > previousEnd + kDevice_RingBufferSize || sampleFrame + inIOBufferFrameSize < previousEnd)
		{
			atomic_store_explicit(&gDevice_FirstWrittenFrame, sampleFrame, memory_order_release);
		}
		atomic_store_explicit(&gDevice_LastWrittenFrame, sampleFrame + inIOBufferFrameSize, memory_order_release);
	}
	else if(inOperationID == kAudioServerPlugInIOOperationReadInput)
	{
		UInt64 firstWritten = atomic_load_explicit(&gDevice_FirstWrittenFrame, memory_order_acquire);
		UInt64 lastWritten = atomic_load_explicit(&gDevice_LastWrittenFrame, memory_order_acquire);
		Boolean hasFreshAudio = !gMute_Input_Master_Value && lastWritten > firstWritten &&
			sampleFrame >= firstWritten && sampleFrame + inIOBufferFrameSize <= lastWritten;

		if(!hasFreshAudio)
		{
			memset(ioMainBuffer, 0, inIOBufferFrameSize * frameBytes);
		}
		else
		{
			memcpy(ioMainBuffer,
				gDevice_LoopbackBuffer + startFrame * channels,
				firstFrames * frameBytes);
			if(wrappedFrames > 0)
			{
				memcpy(((Float32*)ioMainBuffer) + firstFrames * channels,
					gDevice_LoopbackBuffer,
					wrappedFrames * frameBytes);
			}
		}
	}
